...
const PHASES = [
  { name: '吸气', class: 'inhale', duration: 4000 },
  { name: '屏息', class: 'hold', duration: 4000 },
  { name: '呼气', class: 'exhale', duration: 4000 },
  { name: '屏息', class: 'hold-empty', duration: 4000 }
];
...
function playPhaseSound(phaseIndex) {
  ...
  playTone(tones[phaseIndex] || 261.63, 0.4, 'sine', 0.06);
}

function playStartSound() {
  playTone(261.63, 0.2, 'sine', 0.05);
  setTimeout(() => playTone(329.63, 0.2, 'sine', 0.05), 150);
  setTimeout(() => playTone(392.00, 0.3, 'sine', 0.05), 300);
}

function playFinishSound() {
  playTone(392.00, 0.2, 'sine', 0.05);
  setTimeout(() => playTone(493.88, 0.2, 'sine', 0.05), 150);
  setTimeout(() => playTone(587.33, 0.4, 'sine', 0.05), 300);
}
...
function startSession() {
  ...
  rippleRings.forEach(r => r.classList.add('active'));
...
function finishSession() {
  ...
  const rippleRings = document.querySelectorAll('.ripple-ring');
  rippleRings.forEach(r => r.classList.remove('active'));
